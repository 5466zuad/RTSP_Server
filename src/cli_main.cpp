...
void signalHandler(int signum) {
    std::cout << "\n[INFO] Signal(" << signum << ") received, stopping..." << std::endl;
    g_running = false;
}
...
int main(int argc, char** argv) {
    ...
    if (config.input != "camera" && config.input != "file") {
        std::cerr << "[ERROR] --input must be camera or file" << std::endl;
        return 1;
    }

    if (config.input == "file" && config.file.empty()) {
        std::cerr << "[ERROR] file mode requires --file <path> or positional arg" << std::endl;
        return 1;
    }

    std::cout << "[INFO] RTSP server start: bind=" << config.bind_ip
              << " port=" << config.port
    ...
    std::thread serverThread([&server, &config]() {
        if (!server.start(config.bind_ip, config.port, config.stream_path, &g_running)) {
    ...
    std::thread captureThread([&]() {
        if (config.input == "file") {
            std::cout << "[INFO] File streaming: " << config.file << std::endl;
            ...
            std::cout << "[INFO] Camera mode: device=" << config.capture.device
    ...
    std::cout << "[INFO] Waiting for threads..." << std::endl;
